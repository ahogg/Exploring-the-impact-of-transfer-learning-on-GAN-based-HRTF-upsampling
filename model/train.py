...
def train(config, train_prefetcher):
    ...
    if config.single_panel:
        ...
        netD = DiscriminatorSinglePanel(nbins=nbins, hrft_size=config.hrtf_size).to(device)
        ...
        netD = DiscriminatorSingleNode(nbins=nbins, hrft_size=config.hrtf_size).to(device)
    else:
        netG = Generator(upscale_factor=config.upscale_factor, nbins=nbins).to(device)
        netD = Discriminator(nbins=nbins, hrft_size=config.hrtf_size).to(device)
