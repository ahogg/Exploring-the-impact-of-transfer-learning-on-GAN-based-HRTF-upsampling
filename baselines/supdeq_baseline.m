...
% fs = sparseHRTF.fs;
% sparseHRTFdataset = supdeq_sofa2hrtf(SOFAload('sparseHRIRdataset_L38.sofa'),4,nan,4);

% sparseHRTFsofa = SOFAload('sparseHRIRdataset_L38.sofa');
% samplingGrid = sparseHRTFsofa.SourcePosition(:,1:2);
% fs = 48000;
...
NFFT = NFFT*sparseHRTF.FFToversize;
sparseHRTF.f = linspace(0,fs/2,NFFT/2+1);

% HRTF_L = fft(irL,NFFT,2);
% HRTF_R = fft(irR,NFFT,2);
% HRTF_L = HRTF_L(:,1:end/2+1);
% HRTF_R = HRTF_R(:,1:end/2+1);

sparseHRTF.HRTF_L = HRTF_L;
sparseHRTF.HRTF_R = HRTF_R;
